...
3. **Session ended with the eserver VM (192.168.122.200) unreachable** (no route to host, ~4 h of probing from the dev box) — it needs a power-cycle from the hypervisor host. When it's back: deploy `f26540a` (deploy flow above), check `/srv/build/cross` for a stray nested tree from the pre-fix `-oO` run (crossdev's ex-pkg used the overlay's host path, now fixed), and give the cross zstd build one clean run through the job system (`flavor apply -flavor cross` then `build start -flavor cross -package sys-libs/zstd`, expect a signed musl gpkg under `/srv/pkgs/cross/snapshot-*/`).
