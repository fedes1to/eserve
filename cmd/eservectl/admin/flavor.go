...
func PostDeleteFlavor(flavor string) error {
	payload := map[string]string{"flavor": flavor}
	body, _ := json.Marshal(payload)
	response, err := adminClient.Post(urls.SocketURL+urls.FlavorDeleteSuburl, "application/json", bytes.NewReader(body))
	if err != nil {
		return err
	}
	...
	if response.StatusCode != 200 || string(bodyBytes) != "ok" {
		return fmt.Errorf("couldn't delete flavor, code %v, body:\n%v", response.StatusCode, string(bodyBytes))
	}
	return nil
}
